...
const Profile = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                if (error.status == 401) setMessage('Your login session has expired, Please try login again.');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.put('http://localhost:4000/profile/update', 
                formData,
                {
                    headers: {
        ...
        <Container sx={{ maxWidth: 600 }}>
            <Box display="flex" flexDirection="column" alignItems="center" mb={4}>
                <Avatar
                    ...
                    alt="Profile"
                    sx={{ width: 120, height: 120, border: '2px solid #3f51b5', mb: 2 }}
                />
